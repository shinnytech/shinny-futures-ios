//
//  MyBarChartRenderer.swift
//  shinnyfutures
//
//  Created by chenli on 2019/2/18.
...
open class MyBarChartRenderer: BarChartRenderer
{
    ...
    private func prepareBuffer(dataSet: IBarChartDataSet, index: Int)
    {
        ...
        let barWidthHalf = barData.barWidth / 2
    ...
    @objc open override func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        let barWidth = dataProvider.barData?.barWidth ?? 0.01
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            if !isSingleColor
            {
                if let e = dataSet.entryForIndex(j) as? BarChartDataEntry {
                    let sub = e.data as? Double ?? 0.0
                    //线或柱子的颜色
                    if barWidth == 0.01{
                        if sub > 0{
                            context.setStrokeColor(dataSet.color(atIndex: 0).cgColor)
                        }else{
                            context.setStrokeColor(dataSet.color(atIndex: 1).cgColor)
                        }
            ...
            if barWidth == 0.01{
                context.beginPath()
                context.move(to: CGPoint(x: barRect.midX, y: barRect.minY))
                context.addLine(to: CGPoint(x: barRect.midX, y: barRect.maxY))
                context.strokePath()
            }else {
                //成交量为0的话，画一条横线
                if barRect.size.height == 0{
                    context.setStrokeColor(dataSet.color(atIndex: 1).cgColor)
                    context.beginPath()
                    context.move(to: CGPoint(x: barRect.minX, y: barRect.minY))
                    context.addLine(to: CGPoint(x: barRect.maxX, y: barRect.minY))
                    context.strokePath()
                }else if let e = dataSet.entryForIndex(j) as? BarChartDataEntry {
                    let sub = e.data as? Double ?? 0.0
                    if sub > 0{
                        context.fill(barRect)
                    }else{
...
//                setHighlightDrawPos(highlight: high, barRect: barRect)
                ...
                context.setLineWidth(0.7)
                context.beginPath()
                context.move(to: CGPoint(x: xp, y: 0))
                context.addLine(to: CGPoint(x: xp, y: viewPortHandler.contentHeight))
                context.strokePath()

                let y = high.drawY
                let pixels = trans.pixelForValues(x: Double(xp), y: 0)
                let yMax = pixels.y
                let xMax = viewPortHandler.chartWidth
                if y >= 0 && y <= yMax{
                    //画横线
                    context.beginPath()
